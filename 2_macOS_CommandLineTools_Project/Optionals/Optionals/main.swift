...
/*
 ...
 2) ! => We force unwrap and tell Swift that we know the data we are unwraping is how we think it is, so do it.
 ...
 4) if let - else => Here we secify code incase data is the desired one, else handle the error
...
var myName : String?
print(myName?.uppercased()) // prints nill with a warning: "Expression implicitly coerced from 'String?' to 'Any'"
...
print(catAge)   // prints 0 but no error and app does not crash
